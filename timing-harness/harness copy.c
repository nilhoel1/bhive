...
struct perf_event_attr icache_attr = {
    ...
    .sample_type = PERF_SAMPLE_READ,
    .exclude_kernel = 1};
struct perf_event_attr ctx_swtch_attr = {.type = PERF_TYPE_SOFTWARE,
                                         .config =
...
void restart_child(pid_t pid, void *restart_addr, void *fault_addr,
                   int shm_fd) {
...
struct pmc_counters *measure(char *code_to_test, unsigned long code_size,
  ...
  char *aux_mem =
      mmap(NULL, 4096 * 2, PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 4096);
  perror("mmap aux mem");
  ...
  if (pid) { // parent process
    ...
    char *child_mem =
        mmap(NULL, 4096, PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
    struct pmc_counters *counters =
        (struct pmc_counters *)(aux_mem + OFFSET_TO_COUNTERS);
    ...
    *icache_supported = is_event_supported(&icache_attr);
    ...
    for (i = 0; i < unroll_factor; i++) {
      ...
      code_dest += code_size;
      // printf("code_dest after clflush: %p\n", code_dest);
    }
